...
with tempfile.TemporaryDirectory() as td:
 ...
 with sync_playwright() as p:
  ...
  check(page.locator('.nav > button').all_inner_texts()==['Обзор','Карта задач','Динамика','Декларация'],'DOM: main navigation has four expected views')
  check(page.locator('[data-view="discussion"]').count()==0,'DOM: discussion view is absent')
  check(page.locator('.topbar .status-save').count()==0 and 'Сохранено в браузере' not in page.locator('.topbar').inner_text(),'DOM: topbar omits browser save status')
  page.screenshot(path=str(ART/'dashboard-desktop.png'),full_page=True)
  page.locator('[data-goal="g5"]').click();page.locator('#g5s1').check()
  check(page.locator('.drawer-tabs > button').all_inner_texts()==['Шаги','Результат'],'DOM: goal drawer has no discussion tab')
  check(page.locator('[data-tab="comments"]').count()==0,'DOM: comments tab is absent')
  ...
  check(owner.locator('.nav > button').all_inner_texts()==['Обзор','Карта задач','Динамика','Декларация'],'integrated DOM/API: main navigation has four expected views')
  check(owner.locator('[data-view="discussion"]').count()==0,'integrated DOM/API: discussion view is absent')
  check(owner.locator('.topbar .status-save').count()==0,'integrated DOM/API: topbar omits save status')
  ...
  check(guest.locator('.drawer-tabs > button').all_inner_texts()==['Шаги','Результат'],'integrated DOM/API: guest drawer has no discussion tab')
  check(guest.locator('[data-tab="comments"]').count()==0,'integrated DOM/API: guest comments tab is absent')
